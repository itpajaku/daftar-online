...
<form wire:submit="save" class="tab-wizard wizard-circle">
  ...
  <section>
    ...
    <?= Element::withTag('div')
        ...
        ->children([
            Label::create()->for('nama_lengkap')->text('Nama Lengkap *')->class('form-label'),
            Input::create()
    ...
    <?= Element::withTag('div')
        ->class('my-3 form-group')
        ->children([
            Label::create()->for('jenis_kelamin')->text('Jenis Kelamin *')->class('form-label'),
            Select::create()
    ...
    <?= Element::withTag('div')
        ...
        ->children([
            ...
            Input::create()
                ->attribute('wire:model', 'nomor_kependudukan')
                ->placeholder('Contoh : 321...')
                ->name('nomor_kependudukan')
                ->id('nomor_kependudukan')
                ->class('required form-control' . ($errors->has('nomor_kependudukan') ? ' is-invalid' : null))
                ->type('number')
                ->minlength(15),
            $errors->has('nomor_kependudukan') ? Element::withTag('div')->class('invalid-feedback')->text($errors->first('nomor_kependudukan')) : null,
            Element::withTag('div')->attribute('wire:loading')->attribute('wire:target', 'nomor_kependudukan')->class('form-text text-muted')->text('Sedang memeriksa NIK, mohon tunggu...'),
        ]) ?>
    ...
    <?= Element::withTag('div')
        ...
        ->children([
            ...
            Input::create()
                ...
                ->name('nomor_telepon')
                ->id('nomor_telepon')
                ->class('required form-control' . ($errors->has('nomor_telepon') ? ' is-invalid' : null))
                ->type('tel'),
    ...
    <?= Element::withTag('div')
        ...
        ->children([
            ...
            Input::create()
                ->attribute('wire:model', 'email')
                ->type('email')
                ->placeholder('Contoh :user@gmail.com')
                ->name('email')
                ->id('email')
                ->class('required form-control' . ($errors->has('email') ? ' is-invalid' : '')),
    ...
    <?= Element::withTag('div')
        ...
        ->children([
            ...
            Input::create()
                ->attribute('wire:model', 'pekerjaan')
                ->placeholder('Contoh : Karyawan, Ibu Rumah Tangga, Wiraswasta, Pegawai Negeri, dll')
                ->name('pekerjaan')
                ->id('pekerjaan')
                ->class('required form-control' . ($errors->has('pekerjaan') ? ' is-invalid' : '')),
    ...
    <?= Element::withTag('div')
        ...
        ->children([
            ...
            Select::create()
                ->attribute('wire:model', 'pendidikan')
                ->placeholder('-- Pilih Salah Satu --')
                ->name('pendidikan')
                ->id('pendidikan')
                ->options([
                ...
                ->class('required form-select' . ($errors->has('pendidikan') ? ' is-invalid' : '')),
    ...
    <?= Element::withTag('div')
        ...
        ->children([
            ...
            Textarea::create()
                ->attribute('wire:model', 'alamat')
                ->name('alamat')
                ->id('alamat')
                ->placeholder('Masukkan Alamat Lengkap Anda')
                ->class('required form-control ' . ($errors->has('alamat') ? ' is-invalid' : ''))
                ->rows(3)
                ->maxlength(512),
